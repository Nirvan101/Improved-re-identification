...
def network(images1, images2, weight_decay):
    with tf.variable_scope('network'):
        ...
        b , h , w , c = pool1_2.shape
        x = tf.reshape(pool1_2 , [b,c,-1] )
        ...
        def body2(i, j, ai):
            ...
            xij = z[:,i,j]   #(150,)
            xij * aj
            return res  #(150,37,12)
        
        def body1(i, A, out):
            ai = A[:,:,:,i]
            out1 = tf.while_loop(cond2,body2,[i ,j ,ai])    # out = (150,37,12,)
            out = out.stack(out1)
            
        tf.while_loop(cond1,body,[i, A, out])
