...
def network(images1, images2, weight_decay):
    with tf.variable_scope('network'):
        ...
        attention_map_1 = tf.layers.conv2d(pool1_2, 1, [1, 1], activation=tf.nn.relu,
            kernel_regularizer=tf.contrib.layers.l2_regularizer(weight_decay), name='attention_map_1')
        
        attention_map_1_exp = tf.exp(attention_map_1)
        attention_map_1_exp = tf.reduce_sum(attention_map_1_exp, axis=-1)
        
        sume = tf.reduce_sum(attention_map_1_exp, axis=-1)
        sume = tf.reduce_sum(sume, axis=-1)
        sume = tf.expand_dims(sume,axis=-1)
        
        sume = tf.tile(sume, [1, 37])
        sume = tf.expand_dims(sume,axis=-1)
        sume = tf.tile(sume, [1, 1, 12])
        
        attention_map_1_final = tf.divide(attention_map_1_exp , sume)
        attention_map_1_final = tf.expand_dims(attention_map_1_final,axis=-1)
        
        pool1_2 = tf.multiply(pool1_2 , attention_map_1_final)
        print('pool1_2  :')
        print(pool1_2.shape)
        ...
        attention_map_2 = tf.layers.conv2d(pool2_2, 1, [1, 1], activation=tf.nn.relu,
            kernel_regularizer=tf.contrib.layers.l2_regularizer(weight_decay), name='attention_map_2')
        
        attention_map_2_exp = tf.exp(attention_map_2)
        attention_map_2_exp = tf.reduce_sum(attention_map_2_exp, axis=-1)
        
        sume2 = tf.reduce_sum(attention_map_2_exp, axis=-1)
        sume2 = tf.reduce_sum(sume2, axis=-1)
        sume2 = tf.expand_dims(sume2,axis=-1)
        
        sume2 = tf.tile(sume2, [1, 37])
        sume2 = tf.expand_dims(sume2,axis=-1)
        sume2 = tf.tile(sume2, [1, 1, 12])
        
        attention_map_2_final = tf.divide(attention_map_2_exp , sume2)
        attention_map_2_final = tf.expand_dims(attention_map_2_final,axis=-1)
        
        pool1_2 = tf.multiply(pool2_2 , attention_map_2_final)
        print('pool2_2  :')
        print(pool2_2.shape)
